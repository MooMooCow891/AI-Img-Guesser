...
with open(layer_1_neuron, "r") as read_hidden_layer_1:
	#write_hidden_layer_1.writelines(str([[randrange(-100, 101)/100 for i in input_list] for e in range(20)]))
	wei_1 = json.loads(read_hidden_layer_1.read())

with open(layer_2_neuron, "r") as read_hidden_layer_2:
	#write_hidden_layer_2.writelines(str([[randrange(-100, 101)/100 for i in wei_n_bias_1] for e in range(20)]))
	wei_2 = json.loads(read_hidden_layer_2.read())

with open(layer_answer_neuron, "r") as read_layer_answer_neuron:
	#write_layer_answer_neuron.writelines(str([[randrange(-100, 101)/100 for i in wei_n_bias_2]  for e in range(2)]))
	wei_answer = json.loads(read_layer_answer_neuron.read())
...
layer_1 = layer_process(input_list, wei_1)
layer_2 = layer_process(layer_1, wei_2)
layer_ans = layer_process(layer_2, wei_answer)

print(f"Three: {layer_ans[0]}, Bee: {layer_ans[1]}")
print(f'Chosen: {("Three" if layer_ans[0] > layer_ans[1] else "Bee")}')
