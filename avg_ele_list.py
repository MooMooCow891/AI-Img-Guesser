...
def list_get_avg(*inputs: list | tuple) -> list:
	...
	if all(error_list) == False:
		raise ValueError(f'Invalid input{", ".join(s_plural)}: {", ".join(false_inpt)}. Please use lists or tuples instead of {", ".join(type_false_inpt)}')
...
# print(list_get_avg(lis1, lis2, lis3))